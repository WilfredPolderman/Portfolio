    ...
    if (!project) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900">
                <div className="text-center text-2xl text-gray-200">Project not found</div>
                <button
                    onClick={() => navigate(-1)}
                    className="mt-6 flex items-center px-4 gap-2 py-2 bg-gray-700 rounded text-white hover:bg-gray-600"
                >
                    <ArrowLeft size={20} />
                    Go back
                </button>
                ...
                {project.tags && (
                    <div className="flex flex-wrap gap-2 mb-8">
                        {project.tags.map((tag) => (
                            <span
                                key={tag}
                                className="px-4 py-2 bg-emerald-900/30 text-emerald-300 rounded-full text-sm font-medium"
                            >
                                {tag}
                            </span>
                        ))}
